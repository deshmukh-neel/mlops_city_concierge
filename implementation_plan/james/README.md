...
| W2  | Agent loop + ItineraryState + `/chat` endpoint | [w2_agent_graph.md](w2_agent_graph.md) | `feature/agent-w2-agent-graph` | W1 (and ideally W0 for tracing) | ✅ Merged ([#71](https://github.com/deshmukh-neel/mlops_city_concierge/pull/71)) — `commit_itinerary` grounding + manual smoke verified against Cloud SQL |
| W3  | Self-correction (within W2's graph) | [w3_self_correction.md](w3_self_correction.md) | `feature/agent-w3-self-correction` | W2 | ✅ Merged ([#74](https://github.com/deshmukh-neel/mlops_city_concierge/pull/74)) — judge factory now owned by W3; W6 imports `vibe.make_judge` |
| W4  | Booking handoff stub (`propose_booking`) | [w4_booking_stub.md](w4_booking_stub.md) | `feature/agent-w4-booking-stub` | W2 | ✅ Merged ([#75](https://github.com/deshmukh-neel/mlops_city_concierge/pull/75)) — deterministic auto-enrichment instead of LLM tool; Playwright automation deferred |
| W5  | Coverage-gap ingestion agent | [w5_coverage_agent.md](w5_coverage_agent.md) | `feature/agent-w5-coverage-agent` | — (independent) | ✅ Merged ([#77](https://github.com/deshmukh-neel/mlops_city_concierge/pull/77)) — CI SA grant follow-up in [#78](https://github.com/deshmukh-neel/mlops_city_concierge/pull/78); judge default flipped to gemini-3.1-flash-lite-preview |
| W6  | Eval-loop agent (RAGAS retrieval + custom itinerary checks) | [w6_eval_agent.md](w6_eval_agent.md) | `feature/ragas` | W2 | ✅ Merged ([#80](https://github.com/deshmukh-neel/mlops_city_concierge/pull/80)) — RAGAS retrieval + deterministic agent evals; reuses W3 `critique/checks` + `vibe.make_judge` |
| W7  | Knowledge graph (`place_relations`) + `kg_traverse` tool | [w7_knowledge_graph.md](w7_knowledge_graph.md) | `feature/agent-w7-knowledge-graph` | W0a, W1 | ✅ Merged ([#83](https://github.com/deshmukh-neel/mlops_city_concierge/pull/83)) — free/computed edges only; LLM-extracted & editorial-source edges deferred |
| W8  | Live Google Map + multimodal itinerary routing (Tiered Directions re-timing) | [w8_live_map_routing.md](w8_live_map_routing.md) | `feature/agent-w8{a,b,c}-*` | W2, W3 | ✅ Merged — w8a (PR #86), w8b (PR #87, 11/11 real-API gate, 2 prod bugs found+fixed), w8c (PR #88, Tiered Directions re-timing); streaming deferred |
| W9  | Weather-aware recommendations (rain → re-plan outdoor stops) | [w9_weather_aware.md](w9_weather_aware.md) | `feature/agent-w9-weather-aware` | W8c, W3 | 🗓️ Planned / deferred — rides W3 critique loop; core risk is "is this place outdoor?" metadata, not the weather API |
| W10 | LangChain 1.x migration + native Gemini 3 thought signatures | [w10_langchain_v1_gemini3.md](w10_langchain_v1_gemini3.md) | `feature/agent-w10-langchain-v1` | — | 🚧 In progress — rebranched off post-W8 main; root-caused (Gemini 3 never commits: thought-signature bypass hack); fix = lcgg 4.x + LangChain 0.3→1.x, delete gemini_compat (4 call sites on current main, not 1) |
