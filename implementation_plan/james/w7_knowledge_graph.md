...
- **CI SA needs INSERT/DELETE on `place_relations` if integration tests are added.** Follow the pattern W5 established (PR #78 / `alembic/versions/2026_05_08_1100-a1b2c3d4e5f6_grant_ci_sa_proposals.py`): a follow-up alembic migration with a `DO $$ … END $$` block that `EXECUTE`s `GRANT INSERT, DELETE ON place_relations TO "github-actions-deployer@mlops-491820.iam"`, guarded by `IF EXISTS (SELECT 1 FROM pg_roles ...)`. The integration test fixture should also probe `has_table_privilege` — see FUTURE_WATCH "Integration test skip gates should probe privileges, not just existence." If integration tests only *read* `place_relations`, no grant is needed — default `SELECT` on `public` covers it.

---

**Status:** Merged 2026-05-16 in [#83](https://github.com/deshmukh-neel/mlops_city_concierge/pull/83). Shipped: `place_relations` migration + `neighborhood_of()` helper, idempotent builder for 5 free/computed edge types (NEAR, SAME_NEIGHBORHOOD, CONTAINED_IN, NEAR_LANDMARK, SIMILAR_VECTOR), `make build-relations`, real view-aware `kg_traverse` replacing the W2 stub, MLflow `kg_enabled` param, and unit/smoke/functional/integration tests. Verified against live Cloud SQL (5,855 places → 2.0M+ edges, idempotent; `kg_traverse` indexed lookup 0.3 ms). **Deferred:** LLM-extracted edges (`OPERATED_BY`, `MENTIONED_WITH`, `SAME_CHEF`); editorial-source edges; per-source `NEAR` cap; CI-SA `GRANT` migration for integration tests. **Findings:** Cloud SQL IAM users are read-only (migrations/builds need the `postgres` role); the builder is unscoped O(n²) over `places_raw`, making the integration suite slow + non-isolated against production data — follow-up to fixture-scope it.
