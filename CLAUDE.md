...
> **Sync notice:** This file is kept in sync with `AGENTS.md` (Cursor) and `.github/copilot-instructions.md` (GitHub Copilot). When updating project guidance in any of these files, mirror the changes to the other two.
...
- `app` is editable-installed via Poetry (`packages = [{ include = "app" }]` in `pyproject.toml`). Scripts and tests `from app... import ...` directly — **never** re-add a `sys.path.insert`/`REPO_ROOT` bootstrap; `poetry install` makes `app` importable from any cwd
