...
`tests/unit/test_swap.py:498-602`'s shape for `inject_closure_exclusions`,
...
from app.agent.graph import build_agent_graph, inject_primary_type_family
...
def test_inject_primary_type_family_writes_family_into_filters() -> None:
    ...
    out = inject_primary_type_family(
        "semantic_search",
...
def test_inject_primary_type_family_noop_when_requested_empty() -> None:
    ...
    out = inject_primary_type_family("semantic_search", args, [])
    assert out == args
...
def test_inject_primary_type_family_noop_when_slot_index_none_and_query_ambiguous() -> None:
    ...
    args1 = {"query": "ramen"}
    out1 = inject_primary_type_family("semantic_search", args1, ["Sushi Restaurant"])
    assert out1 == args1
    assert "filters" not in out1
    # slot_index explicitly None
    args2 = {"query": "ramen", "slot_index": None}
    out2 = inject_primary_type_family("semantic_search", args2, ["Sushi Restaurant"])
    assert out2 == args2
...
def test_inject_primary_type_family_fallback_infers_from_query_without_slot_index() -> None:
    ...
    out2 = inject_primary_type_family(
...
def test_inject_primary_type_family_noop_when_tool_name_not_retrieval() -> None:
    ...
    for name in ("commit_itinerary", "kg_traverse", "get_details"):
        args = {"query": "x", "slot_index": 0}
        out = inject_primary_type_family(name, args, ["Sushi Restaurant"])
...
def test_inject_primary_type_family_result_is_json_dumps_safe() -> None:
    ...
    for filters_in in filters_shapes:
        ...
        out = inject_primary_type_family("semantic_search", base_args, requested)
...
def test_inject_primary_type_family_does_not_mutate_input_args() -> None:
    ...
    out = inject_primary_type_family("semantic_search", args, ["Sushi Restaurant"])
...
class OneShotLLM(BaseChatModel):
    ...
    def bind_tools(self, tools: Any, **kwargs: Any) -> OneShotLLM:
        return self
...
async def drive_act_once(
    ...
    def record(name: str, args: dict[str, Any]) -> list[Any]:
    ...
    monkeypatch.setattr(
        ...
        lambda **kw: record("semantic_search", kw),
    )
    monkeypatch.setattr(
        "app.agent.tools.nearby_impl",
        lambda **kw: record("nearby", kw),
    )
...
async def test_act_strips_slot_index_before_tool_invoke(
    ...
    assert invokes, "underlying semantic_search_impl was never invoked"
    # `semantic_search_impl` is called with keyword args from the tool wrapper
...
async def test_act_does_not_alter_kg_traverse_args_when_strip_runs(
    ...
    def kg(**kw: Any) -> list[Any]:
        captured.append(dict(kw))
        return []

    monkeypatch.setattr("app.tools.graph.kg_traverse", kg)

    # Drive kg_traverse via the same _OneShotLLM. closure_context is empty so
    # inject_closure_exclusions is also a noop — effective_args must equal
    ...
    graph = build_agent_graph(OneShotLLM(scripted=ai), max_steps=4)
    out = await graph.ainvoke(state)
    ...
    recorded = entries[0]["args"]
    closure_only = inject_closure_exclusions(
        "kg_traverse",
...
async def test_act_does_not_mutate_tc_args_under_slot_injection(
    ...
    monkeypatch.setattr("app.agent.tools.semantic_search_impl", lambda **kw: [])
    graph = build_agent_graph(OneShotLLM(scripted=ai), max_steps=4)
    out = await graph.ainvoke(state)
