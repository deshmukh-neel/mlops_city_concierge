...
def test_conversation_uses_history_when_current_message_lacks_count() -> None:
    history = [Msg("user", "plan a 3-stop omakase date night"), Msg("assistant", "Done.")]
...
def test_conversation_ignores_assistant_messages() -> None:
    ...
    history = [Msg("user", "plan a date"), Msg("assistant", "I'll find 5 spots")]
...
def test_conversation_latest_user_count_wins_when_user_revises_twice() -> None:
    history = [
        Msg("user", "plan a 3-stop omakase date night"),
        Msg("assistant", "Done."),
        Msg("user", "actually 5 places"),
...
def test_conversation_returns_none_when_neither_history_nor_message_has_count() -> None:
    history = [Msg("user", "plan a date"), Msg("assistant", "How many?")]
...
def test_conversation_bare_number_after_stops_question_is_count() -> None:
    ...
    history = [
        Msg("user", "plan me a date in mission, gf likes omakase"),
        Msg(
            "assistant",
...
def test_conversation_bare_word_number_after_stops_question_is_count() -> None:
    ...
    history = [
        Msg("user", "plan a date"),
        Msg("assistant", "Sure — how many stops would you like?"),
    ]
...
def test_conversation_bare_number_ignored_when_assistant_didnt_ask_about_stops() -> None:
    ...
    history = [
        Msg("user", "plan a date"),
        Msg("assistant", "Got it. What neighborhood?"),
    ]
...
def test_conversation_bare_number_out_of_range_ignored() -> None:
    ...
    history = [
        Msg("user", "plan a date"),
        Msg("assistant", "How many stops would you like?"),
    ]
...
def test_conversation_bare_number_after_question_loses_to_history_explicit_count() -> None:
    ...
    history = [
        Msg("user", "plan a 3-stop date"),
        Msg("assistant", "Done."),
        Msg("user", "make it different"),
        Msg("assistant", "Sure, how many stops do you want now?"),
    ]
...
def test_has_slot_structure_regexes_compile_at_module_load() -> None:
    ...
    from app.agent import input_parsing as input_parsing_mod

    assert isinstance(input_parsing_mod.THEN_PATTERN_RE, re_mod.Pattern)
    assert isinstance(input_parsing_mod.NUMBERED_SLOT_RE, re_mod.Pattern)
    assert isinstance(input_parsing_mod.SLOT_VOCAB_RE, re_mod.Pattern)
