...
def test_redact_removes_openai_key() -> None:
    """D-10-13: redact removes OpenAI sk- key patterns."""
    from scripts.probe_provider_capture import redact
...
def test_redact_removes_anthropic_key() -> None:
    """D-10-13 / IN-04: redact removes Anthropic sk-ant- key patterns.

    Per PATTERNS.md test_probe_redaction_catches_anthropic_key:
        assert 'sk-ant-REDACTED' in redact('sk-ant-api03-abc123xyz789...')
        assert 'sk-ant-api03-abc123xyz789' not in redact(...)
    """
    from scripts.probe_provider_capture import redact
...
def test_redact_removes_google_key() -> None:
    """D-10-13: redact removes Google AIzaSy... key patterns (33 chars after prefix)."""
    from scripts.probe_provider_capture import redact
...
def test_redact_removes_env_sourced_openai_key() -> None:
    """D-10-13: redact substitutes env-var-sourced secret values regardless of position."""
    ...
    with patch.dict(os.environ, {"OPENAI_API_KEY": fake_key}):
        ...
        result = probe_mod.redact(f"some text containing {fake_key} in it")
        assert fake_key not in result
        assert "<REDACTED>" in result


def test_redact_leaves_benign_text_unchanged() -> None:
    """redact does not mangle non-secret text."""
    from scripts.probe_provider_capture import redact

    benign = "The weather is nice today. model=gpt-4o-mini"
    result = redact(benign)
    assert result == benign


def test_secret_patterns_list_covers_expected_providers() -> None:
    """SECRET_PATTERNS covers OpenAI, Anthropic, and Google patterns (D-10-13)."""
    from scripts.probe_provider_capture import SECRET_PATTERNS

    assert len(SECRET_PATTERNS) >= 3, (
        f"Expected at least 3 secret patterns (OpenAI/Anthropic/Google); got {len(SECRET_PATTERNS)}"
    )
    ...
    openai_matched = any(p.search(openai_key) for p in SECRET_PATTERNS)
    anthropic_matched = any(p.search(anthropic_key) for p in SECRET_PATTERNS)
    google_matched = any(p.search(google_key) for p in SECRET_PATTERNS)
...
def test_post_write_guard_rejects_fixture_with_planted_secret(tmp_path: Path) -> None:
    ...
    found = any(p.search(text) for p in SECRET_PATTERNS)
    assert found, (
...
def test_fixture_output_path_uses_provider_payloads(tmp_path: Path) -> None:
    ...
    from scripts.probe_provider_capture import fixture_path
...
def test_post_write_guard_catches_env_var_sourced_secret(tmp_path: Path) -> None:
    ...
    Plants a rotated DeepSeek key that does NOT match any SECRET_PATTERNS regex
    ...
    (scan_fixture_for_secrets) and asserts:
    ...
    # This must NOT match any SECRET_PATTERNS regex so the test proves the env-var
    ...
    with patch.dict(os.environ, {"DEEPSEEK_API_KEY": rotated_secret}):
        ...
        # Confirm the secret does NOT match any SECRET_PATTERNS regex (if it does,
        # the test is testing the regex path, not the env-var path).
        secret_matches_regex = any(p.search(rotated_secret) for p in probe_mod.SECRET_PATTERNS)
        assert not secret_matches_regex, (
            f"Rotated secret {rotated_secret!r} matched a SECRET_PATTERNS regex — "
        ...
        # The production helper scan_fixture_for_secrets must detect the secret.
        text = fixture_file.read_text(encoding="utf-8")
        found = probe_mod.scan_fixture_for_secrets(text)
        assert found, (
            "scan_fixture_for_secrets should detect the env-var-sourced secret "
