...
must_haves:
  truths:
    ...
    - "The smoke/functional stub exercises the ROUND-3 cuisine-recall path: the stubbed `user_query_log` demand row has `requested_primary_types=[]` (the free-text case `app/main.py` actually produces) and a `message` naming a catalog cuisine + neighborhood, so the populated path proves the cuisine is recovered from the message via Plan 02's `_lexical_cuisines` fallback — the functional layer fails if the cuisine-recall HIGH fix regresses (REVIEW ROUND-3 HIGH)."
    - "The smoke/functional checkpoint stub returns a `status` column and the `FROM places_ingest_query_checkpoints` branch reflects Plan 03's `WHERE status = 'completed'` filter: the stub returns a COMPLETED prefixed `all::...` checkpoint row that does NOT normalize to the mined seed (so the proposal survives dedup), confirming the smoke path tolerates BOTH the status filter and the prefix strip (REVIEW ROUND-2 NEW HIGH + ROUND-3 MEDIUM)."
    - "The integration test is DETERMINISTIC and STATEFUL-SAFE (REVIEW MEDIUM): it forces a gap by passing a high `--min-places` so the seeded bucket is guaranteed under threshold, and it selects a catalog `(neighborhood, cuisine)` pair that is absent from BOTH `places_ingest_query_proposals` AND the NORMALIZED completed-checkpoint set BEFORE asserting it gets inserted — so the checkpoint-prefix normalization (Plan 03 ROUND-2 NEW HIGH) and the completed-only status filter (ROUND-3 MEDIUM) cannot cause the target pair to be silently deduped (REVIEW ROUND-2 MEDIUM-1). It cleans up ONLY the rows it inserted (tracked by a unique rationale marker / returned id) — it NEVER blanket-deletes by `query_text`."
    - "Integration tests are skipped unless `APP_ENV=integration` (project convention) and seed the sandbox via the Plan 01 demand-seed helper, then assert a real `pending` proposal row in `places_ingest_query_proposals` with the exact loop-consumable seed format (D-03). If no catalog pair is free of both proposals and normalized completed-checkpoints, the test SKIPS with an explicit reason rather than asserting against a pre-deduped pair (REVIEW ROUND-2 MEDIUM-1)."
  ...
  artifacts:
    - path: "tests/unit/test_gap_miner_smoke.py"
      provides: "Smoke + functional tests for the demand-mining entrypoint (cuisine-recall path + completed-checkpoint stub)"
    - path: "tests/integration/test_gap_miner.py"
      provides: "Deterministic, scoped-cleanup integration test: seeded sandbox demand → real pending proposal, picks a pair free of proposals AND normalized completed-checkpoints"
    - path: "Makefile"
...
Purpose: The project owner's non-negotiable test-layering preference requires unit/mock + smoke + functional + integration for new modules. Plans 02/03 delivered unit/mock; this plan adds the remaining three layers and the operator-facing ergonomics (Make target + docs). The integration test is the honest end-to-end proof that seeded demand produces a real loop-consumable proposal row (GAP-03) — made DETERMINISTIC and STATEFUL-SAFE per the cross-AI review MEDIUMs, made robust to the checkpoint-prefix normalization AND the completed-only status filter by choosing a target pair that is absent from both proposals AND the normalized completed-checkpoint set (ROUND-2 MEDIUM-1 + ROUND-3 MEDIUM), and the functional layer now exercises the ROUND-3 cuisine-recall path (free-text row with empty `requested_primary_types`).

Output: `tests/unit/test_gap_miner_smoke.py` (smoke + functional, cuisine-recall + completed-checkpoint stubs), `tests/integration/test_gap_miner.py` (real-DB, deterministic, scoped cleanup, checkpoint-aware target selection), `make gap-mine`, and the three synced docs.
</objective>
...
<task type="auto" tdd="true">
  ...
  <read_first>
    ...
    - scripts/coverage_agent.py (the demand entrypoint `gap_mine_main` + `gather_demand`/`gather_pair_supply`/`find_demand_gaps`/`ingested_query_texts`/`log_to_mlflow` from Plans 02/03; the `_StubCursor` must now branch for each distinct SELECT — user_query_log rows for the demand SELECT (`FROM user_query_log`), pair-supply rows for the pair-supply SELECT (`FROM place_query_hits`), COMPLETED prefixed checkpoint rows for `ingested_query_texts`'s checkpoint SELECT (`FROM places_ingest_query_checkpoints` — now carries `WHERE status = 'completed'`), and an explicit empty result for `ingested_query_texts`'s proposals SELECT (`FROM places_ingest_query_proposals`) — so the reads don't cross-contaminate and BOTH `ingested_query_texts` branches are exercised)
    - app/main.py lines ~64-88 (the slot-intake prompt returns `requested_primary_types=[]` for free-text — the stubbed demand row MUST use empty `requested_primary_types` + a message naming a catalog cuisine so the functional layer exercises Plan 02's `_lexical_cuisines` fallback — ROUND-3 HIGH)
    ...
    - .planning/phases/18-gap-mining-gap/18-REVIEWS.md § HIGH-1/HIGH-2 + ROUND-2 NEW HIGH + ROUND-3 NEW HIGH (cuisine recall) + ROUND-3 MEDIUM (checkpoint status) (so the functional stubs exercise pair-supply, the ingested-only/prefix-normalized/completed-only dedup, AND the message-cuisine recall — the checkpoint stub returns a COMPLETED PREFIXED `all::...` row to confirm the smoke path tolerates it)
  </read_first>
  <behavior>
    ...
    - Functional 1 (ROUND-3 cuisine recall): with a stubbed connection returning a user_query_log row with `requested_primary_types=[]` (empty — the free-text case) and a `message` naming a catalog cuisine + neighborhood ("vietnamese restaurants in Outer Sunset"), stubbed `place_query_hits` showing that bucket's pair supply below min_places, ingested-query reads returning a COMPLETED PREFIXED `all::...` checkpoint row that does NOT match the mined seed (so it survives dedup — HIGH-2 + ROUND-2 NEW HIGH + ROUND-3 status), and a stub LLM, `gap_mine_main(["--dry-run","--days","1"])` returns 0 and `mlflow.log_dict` is called with a `demand_gaps.json` artifact containing the ranked `(Outer Sunset, vietnamese)` gap — proving the cuisine was recovered from the message (the cuisine-recall HIGH fix).
  ...
  <action>
    Create `tests/unit/test_gap_miner_smoke.py` mirroring `tests/unit/test_coverage_agent_smoke.py`'s structure. Extend the stub cursor to branch on SQL substrings: return ONE seeded demand tuple `("vietnamese restaurants in Outer Sunset", [])` (`message` naming a catalog cuisine + neighborhood, `requested_primary_types` EMPTY — the free-text case `app/main.py` produces) when the executed SQL contains `FROM user_query_log` (this exercises Plan 02's `_lexical_cuisines` message-cuisine recall — ROUND-3 HIGH); return pair-supply rows (`query_text`, count below min_places) when it contains `FROM place_query_hits`; return checkpoint rows (a `(query_text,)` or `(query_text, status)` tuple matching the columns Plan 03's `ingested_query_texts` selects) when it contains `FROM places_ingest_query_checkpoints` — return a COMPLETED PREFIXED `all::...` checkpoint string that does NOT normalize to the mined seed, so the proposal survives `ingested_query_texts` dedup (HIGH-2 + ROUND-2 NEW HIGH); the checkpoint SELECT now carries `WHERE status = 'completed'`, so ensure the stub returns the row for that filtered query; return an explicit empty list `[]` when it contains `FROM places_ingest_query_proposals` (so `ingested_query_texts`'s second SELECT branch is exercised, not silently caught by the default); return `[]` otherwise. Monkeypatch `vibe.make_judge` (stub LLM returning a no-op / empty extraction for any LLM-miss step — the lexical pre-passes should fully cover this row, so the LLM must not be required), monkeypatch `scripts.sandbox_guard.assert_sandbox_write_target` (and/or its import in coverage_agent) to a no-op (unit context has no real sandbox), and monkeypatch all `mlflow.*` calls. Cover the four behaviors above: module smoke (assert all the demand names present + the guard importable), cold-start smoke, populated functional with `demand_gaps.json` artifact assertion proving the `(Outer Sunset, vietnamese)` gap was recovered from the message, and dry-run-no-write. Do NOT hit a real DB or LLM.
  </action>
  <verify>
  ...
  <acceptance_criteria>
    ...
    - A functional test asserts the populated path with a stubbed demand row that has EMPTY `requested_primary_types` and a `message` naming a catalog cuisine — proving the cuisine is recovered from the message (ROUND-3 HIGH) — and a COMPLETED PREFIXED `all::...` checkpoint stub that does NOT match the mined seed, logs a `demand_gaps.json` ranked artifact containing the `(Outer Sunset, vietnamese)` gap, and returns 0.
  ...
  <done>Smoke + functional layers exist: the demand entrypoint imports, cold-starts cleanly, and walks the populated path with the ROUND-3 cuisine-recall stub (empty `requested_primary_types`, cuisine from message) + completed/prefix-normalized checkpoint dedup, emitting demand artifacts — all with stubs, no real services.</done>
</task>
...
<task type="auto">
  ...
  <read_first>
    ...
    - scripts/coverage_agent.py (`gap_mine_main` — the entrypoint under test; `gap_to_seed_query` for the expected proposal text; `ingested_query_texts(conn)` — the NORMALIZED completed-only dedup set the test uses to verify the chosen target pair is not already covered; `--min-places` for forcing the gap; `_lexical_cuisines`/`_lexical_neighborhoods` — the lexical pre-passes that make the seeded row map without an LLM)
    - scripts/ingest_places_sf.py (NEIGHBORHOODS / CUISINES / build_seed_queries — the catalog the test iterates to find a pair whose seed is absent from both proposals and normalized completed-checkpoints)
    ...
    - .planning/phases/18-gap-mining-gap/18-REVIEWS.md § MEDIUM "Integration-test determinism" + ROUND-2 "MEDIUM-1" + ROUND-3 MEDIUM (checkpoint status) (authoritative: force the gap with high --min-places; choose a catalog pair absent from BOTH proposals AND the NORMALIZED completed-checkpoint set so neither the prefix normalization nor the completed-only status filter can pre-dedupe the target; clean up ONLY test-inserted rows by unique marker; skip with a clear reason if no clean pair exists)
  ...
  <action>
    Create `tests/integration/test_gap_miner.py` gated on `APP_ENV=integration` (mirror `test_coverage_agent.py`'s `pytestmark`). Add a `_user_query_log_or_skip` fixture mirroring `_proposals_table_or_skip` that skips when `user_query_log` is absent OR the role lacks INSERT on it (so the test starts passing once the sandbox/CI DB has the Phase 17 migration — the known shared-DB skip pattern from memory project_full_suite_db_pool_contamination and the existing integration test). The test, in order: (1) builds the NORMALIZED dedup set via `ingested_query_texts(conn)` (which filters checkpoints to `status='completed'` per ROUND-3 MEDIUM and strips the `FIELD_MODE::` checkpoint prefix per Plan 03 ROUND-2 NEW HIGH) AND reads the current `places_ingest_query_proposals` query_texts, then iterates `[(n, c) for n in NEIGHBORHOODS for c in CUISINES]` to pick the FIRST pair whose `gap_to_seed_query(n, c)` is in `set(build_seed_queries())` (catalog-valid) AND is absent from BOTH the normalized dedup set AND the existing proposals — if NO such pair exists, `pytest.skip` with an explicit reason ("no catalog pair free of proposals and normalized completed-checkpoints") rather than asserting against a pre-deduped pair (REVIEW ROUND-2 MEDIUM-1 + ROUND-3 MEDIUM); (2) records the expected seed `query_text = gap_to_seed_query(chosen_n, chosen_c)` and confirms (defensively) no pre-existing proposal row has that text (so cleanup never deletes a legitimate pre-existing proposal — REVIEW MEDIUM); (3) seeds demand rows for the CHOSEN pair via `insert_demand_rows` with a UNIQUE per-run rationale/marker AND a `message` containing the chosen neighborhood + the chosen cuisine word (so the demand path maps it lexically — both the neighborhood via `_lexical_neighborhoods` and the cuisine via `_lexical_cuisines` — with NO LLM dependency, exercising the ROUND-3 cuisine-recall path against a real DB); (4) stubs `vibe.make_judge` so any LLM-miss step is deterministic — but the lexical pre-passes on BOTH axes should already cover the seeded row; (5) stubs mlflow; (6) runs `gap_mine_main(["--days","30","--min-places","100000"])` — the deliberately huge `--min-places` FORCES the seeded bucket under threshold so the gap is GUARANTEED regardless of real sandbox supply (REVIEW MEDIUM determinism); (7) asserts a real `pending` row exists in `places_ingest_query_proposals` whose `query_text == gap_to_seed_query(chosen_n, chosen_c)`; (8) in a `finally`, cleans up ONLY the rows the test created — delete the seeded demand rows by their unique marker, and delete the proposal row ONLY because step (1)/(2) confirmed it did NOT pre-exist (track by the row id returned at insert if available, else by the exact chosen query_text). NEVER blanket-delete by query_text when a pre-existing proposal was present. Use parameterised DELETEs for cleanup (no string interpolation). Mirror `_purge_test_proposals` for the marker-scoped delete.
  </action>
  <verify>
  ...
  <acceptance_criteria>
    ...
    - The test selects a catalog `(neighborhood, cuisine)` pair absent from BOTH `places_ingest_query_proposals` AND the NORMALIZED completed-checkpoint set from `ingested_query_texts(conn)`, or `pytest.skip`s with an explicit reason when none is free (REVIEW ROUND-2 MEDIUM-1 + ROUND-3 MEDIUM — neither the prefix normalization nor the completed-only filter can silently pre-dedupe the target).
    - The seeded demand row uses a `message` naming the chosen cuisine + neighborhood so the demand maps lexically on BOTH axes with no LLM (exercising the ROUND-3 cuisine-recall path against a real DB).
  ...
  <done>A deterministic integration test proves seeded sandbox demand produces a real loop-consumable pending proposal (GAP-03 end-to-end) using a forced gap and a target pair guaranteed free of proposals AND normalized completed-checkpoints (ROUND-2 MEDIUM-1 + ROUND-3 MEDIUM), seeds via a message that maps lexically on both axes (ROUND-3 cuisine recall), scopes its cleanup to only test-created rows, and skips gracefully on a DB without the Phase 17 migration.</done>
</task>
...
| make gap-mine → DB | Operator-run; writes proposals to the pool (`DATABASE_URL` = sandbox), enforced by the `current_database()`-based `assert_sandbox_write_target` run on the same conn as the insert |

## STRIDE Threat Register

| Threat ID | Category | Component | Disposition | Mitigation Plan |
|-----------|----------|-----------|-------------|-----------------|
| T-18-04-INT | Tampering | integration test seed/cleanup | mitigate | Test is gated on `APP_ENV=integration`, FORCES the gap deterministically (high `--min-places`), picks a target pair absent from BOTH proposals AND the NORMALIZED completed-checkpoint set (ROUND-2 MEDIUM-1 + ROUND-3 MEDIUM) or skips with a clear reason, detects pre-existing proposals, and purges ONLY test-created demand + proposal rows in a `finally` with parameterised DELETEs scoped by unique marker — never a blanket query_text delete (REVIEW MEDIUM). Skips gracefully when the table is absent (shared-DB safety). |
| T-18-04-WRITE | Elevation of Privilege | `make gap-mine` write target | mitigate | The target invokes `gap_mine_main`, which calls `assert_sandbox_write_target(conn)` (shared `scripts.sandbox_guard`, checks live `current_database()`) on the SAME conn before `insert_pending(..., conn=conn)` (Plan 03, REVIEW HIGH-3 + ROUND-2 H3 + ROUND-3 LOW) — writes go via the pool → `DATABASE_URL` and are refused unless the resolved dbname is the sandbox. No prod `places_raw` write path. Docs state the write target is the sandbox. |
...
- The smoke/functional layer exercises the ROUND-3 cuisine-recall path (empty `requested_primary_types`, cuisine recovered from the message) and the completed/prefix-normalized checkpoint dedup, so a regression of either fix fails a fast unit-level test.
- GAP-03 proven end-to-end: seeded sandbox demand → real pending proposal in exact seed format, via a deterministic forced-gap test that picks a target pair free of proposals AND normalized completed-checkpoints (ROUND-2 MEDIUM-1 + ROUND-3 MEDIUM) with scoped cleanup (REVIEW MEDIUM).
