...
1. **Exact supply join for neighborhood+cuisine cross-buckets**
   ...
   - RESOLVED (per-cuisine) — **SUPERSEDED by Phase 18 cross-AI review HIGH-1.** The original resolution used per-cuisine supply for the gate, but that misses the canonical demand case Phase 18 exists to surface: a cuisine present city-wide but ABSENT in the demanded neighborhood (e.g. Vietnamese everywhere in SF, zero in Outer Sunset). **Corrected resolution (implemented in 18-03):** score supply at the TRUE `(neighborhood, cuisine)` PAIR level via `gather_pair_supply`, which counts `COUNT(DISTINCT place_id) FROM place_query_hits WHERE query_text = "{cuisine} restaurants in {neighborhood} San Francisco"` for each demanded pair (never-ingested pairs → 0, correctly flagged). `places_raw` has no clean neighborhood column (only the free-text `formatted_address` regex used by `gather_stats` and `source_query`, which is not refreshed on upsert), so `place_query_hits.query_text` — the indexed query→place evidence table whose seed text encodes both catalog axes — is the authoritative pair-supply signal. The `min(nbhd_supply, cuisine_supply)` code example earlier in this doc is NOT used.
