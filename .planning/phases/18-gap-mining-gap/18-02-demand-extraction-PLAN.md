...
must_haves:
  truths:
    ...
    - "Judge absence does NOT suppress demand: when `vibe.make_judge()` returns None, every row whose neighborhood resolved via the lexical pre-pass STILL produces a `(neighborhood, cuisine)` demand tuple; only the lexical-MISS rows (which needed the LLM) degrade to empty and are counted in `unmapped_count`. This is the upstream half of the judge-absence semantics consumed by Plan 03's cold-start logic (REVIEW ROUND-2 MEDIUM-3 — judge None ≠ blanket no demand)."
  ...
  artifacts:
    ...
    - path: "tests/unit/test_gap_miner.py"
      provides: "Unit tests for the demand-extraction path (lexical cuisine map, lexical-then-LLM multi-neighborhood extraction, batch LLM call-count, catalog constraint, unmapped_count, multi-intent demand, judge-None-still-maps-lexical)"
  key_links:
...
Purpose: This is GAP-01 — Phase 18's distinctive contribution. `coverage_agent` already does supply-side gap detection; what it lacks is a real demand signal. This plan adds that signal additively without touching any existing supply-only function or its W5 tests (the guardrail). It incorporates the review's extraction MEDIUMs: neighborhood extraction is lexical-before-LLM (works without judge creds) and returns a LIST per row (multi-intent messages count toward every neighborhood they name). Per ROUND-2 MEDIUM-3, judge absence must NOT suppress lexically-mappable demand — that is the upstream contract Plan 03's cold-start logic relies on.
...
<task type="auto" tdd="true">
  ...
  <read_first>
    ...
    - .planning/phases/18-gap-mining-gap/18-REVIEWS.md § MEDIUMs "Extraction shape conflict" + "Lexical-before-LLM neighborhood matching" + ROUND-2 "MEDIUM-3" (authoritative: return a LIST of neighborhoods per row; do exact lexical NEIGHBORHOODS matching before the LLM hop so extraction works without judge creds; judge None must still map lexical hits)
  ...
  <behavior>
    ...
    - Test 4 (lexical-only works without creds — REVIEW MEDIUM + ROUND-2 MEDIUM-3): with `llm=None` (missing judge creds), a message containing an exact catalog neighborhood name still maps via the lexical pre-pass (returns that neighborhood); only lexical-miss rows degrade to empty/unmapped. Assert the lexical-hit message yields its neighborhood even when `llm is None`.
  ...
  <acceptance_criteria>
    ...
    - `_lexical_neighborhoods` maps an exact-catalog-name message with NO LLM call, returns a LIST (multi-neighborhood), and works when `llm` is unavailable (REVIEW MEDIUM — lexical-before-LLM; ROUND-2 MEDIUM-3 — Test 4 asserts a lexical hit still maps with `llm=None`).
...
<task type="auto" tdd="true">
  ...
  <read_first>
    ...
    - .planning/phases/18-gap-mining-gap/18-REVIEWS.md § MEDIUM "Extraction shape conflict" + ROUND-2 "MEDIUM-3" (multi-neighborhood: a row contributes demand to EACH (neighborhood, cuisine) it mentions; judge None still counts lexically-mapped rows)
  </read_first>
  <behavior>
  ...
  <acceptance_criteria>
    ...
    - Judge None still yields non-empty `demand_counts` for lexically-resolved rows; only lexical-miss rows become unmapped (ROUND-2 MEDIUM-3 — Test 8).
  ...
  <done>`gather_demand` reads `user_query_log` (pool or DEMAND_DATABASE_URL), returns catalog-constrained demand counts + rows_scanned + unmapped_count via lexical cuisine map + lexical-then-single-batched-LLM multi-neighborhood extraction (multi-intent rows count every pair; judge None still counts lexical hits — ROUND-2 MEDIUM-3); supply-only path untouched; tests green.</done>
</task>
...
- Neighborhood extraction returns a LIST per row and works without judge creds for exact catalog names (REVIEW MEDIUMs: extraction-shape + lexical-before-LLM); judge None still counts lexically-mapped demand (ROUND-2 MEDIUM-3 — the upstream half Plan 03's cold-start logic relies on); multi-intent rows count every pair.
