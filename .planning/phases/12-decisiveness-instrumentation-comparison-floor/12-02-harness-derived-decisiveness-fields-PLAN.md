...
must_haves:
  truths:
    ...
    - "step_telemetry from ItineraryState is forwarded verbatim into the run JSON"
  artifacts:
...
<task type="auto">
  ...
  <read_first>
    ...
    - app/agent/graph.py (act() lines 340-415: commit_itinerary scratch entries are dicts
      with a `"step"` key written at `state.step_count`; semantic_search/nearby entries have
      `result` = list of hits and a `"step"` key)
    - .planning/phases/12-decisiveness-instrumentation-comparison-floor/12-PATTERNS.md
  ...
  <action>
    ...
    -> list[int]` — for each plan step, count hits across `semantic_search` and `nearby`
    scratch entries (grouped by their `"step"` key) whose `value_from_hit(hit,
    "similarity")` is a number >= `viability_threshold` AND whose `value_from_hit(hit,
    "primary_type")` is in `requested_types` (or `requested_types` is empty → no type
    constraint, count on cosine alone). Return one int per step index in step order.
    (3) `rule8_met_per_step_from_state(state, viable_per_step, requested_types) ->
    list[bool]` — per step, True iff every requested stop already had >=1 viable candidate
    accumulated up to and including that step (per D-12-05). When `requested_types` is empty,
    use the count of distinct requested slots from `state.constraints.num_stops` if set,
    else treat "at least one viable candidate exists" as the precondition. Guard every read
    with isinstance checks like the existing helpers (scratch entries may be malformed).
  </action>
  <verify>
  ...
  <done>Three pure helpers plus seven new DeterministicEvalResult fields exist; threshold is imported, never hardcoded.</done>
</task>

<task type="auto">
  <name>Task 2: Wire derived fields into query_result_from_state + make_error_record defaults</name>
  <files>scripts/eval_agent.py</files>
  <read_first>
  ...
  <action>
    ...
    `asdict()`-clean (no non-serializable values).
  </action>
  <verify>
  ...
  <done>Both record-assembly paths populate the seven INST fields; run JSON is self-describing and asdict/json-safe.</done>
</task>
...
<task type="auto">
  ...
  <read_first>
    ...
    - scripts/eval_agent.py (the three helpers + their exact viability/rule-8 semantics from
      Task 1, to write assertions that match the implementation)
    - .planning/phases/12-decisiveness-instrumentation-comparison-floor/12-PATTERNS.md
  ...
  <action>
    ...
    `similarity >= LOW_SIMILARITY_THRESHOLD` AND matching `primary_type`; assert that with
    an empty `requested_types` the count falls back to cosine-only; assert a `nearby` entry
    with `similarity=0.0` contributes zero viable candidates. Tests for
    `rule8_met_per_step_from_state`: with two requested types and viable candidates for both
    accumulated, the per-step boolean is True; with only one type covered, it is False.
    Assert results are JSON-safe (`json.dumps` of each helper output succeeds).
  </action>
  <verify>
  ...
  <acceptance_criteria>
    ...
    - A test asserts `rule8_met_per_step_from_state` is False when only one of two requested types has a viable candidate
    - `poetry run pytest tests/unit/test_eval_agent.py -q` passes with no new failures
  </acceptance_criteria>
  <done>Unit tests cover the commit-step, viability, and rule-8 helpers including the cosine/type/empty-type edge cases.</done>
</task>
...
- A run JSON built from a committed state contains numeric first_commit_call_step, a viable_candidates_per_step list, a rule8_met_per_step list, and viability_threshold == LOW_SIMILARITY_THRESHOLD — all readable directly (no post-processing, D-12-02)
