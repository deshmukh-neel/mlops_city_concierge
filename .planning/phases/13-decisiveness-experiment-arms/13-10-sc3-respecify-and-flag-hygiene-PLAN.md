...
<task type="auto">
  ...
  <acceptance_criteria>
    ...
    - The module-level `_VIABILITY_CONTRACT_ENABLED` constant is removed: `grep -rn "_VIABILITY_CONTRACT_ENABLED" app/ scripts/` returns no matches (tests may reference it only in comments/history, not as a live import or assertion).
